...
mycolumns=['REVIEW_URL','SKU','COUNTRY','SITE_URL','REVIEW_POSTED_DATE','STAR_RATING','REVIEW_FULL_TEXT','PRODUCT_TYPE','PRODUCT_GROUP','PRODUCT_LINE_NAME']
...
end_day=newenddate-timedelta(days=2)
...
#myque=make_quer_date(mycolumns,2017,8,12,2,2017,9,24)
cur.execute(myque)
...
def extract_reviewid_url(reviewurl):
    sreviewurl=str(reviewurl)
    lr=len(sreviewurl)
    prid=sreviewurl.rfind('/')
    extracted_id=sreviewurl[prid+1:]
...
#         print 'Yoo'
    row_l=list(row)
    row_l[0]=extract_reviewid_url(row_l[0])
    #print(row_l)
...
df_aws_sentences[['sentence_id','word_count','star_rating']]=df_aws_sentences[['sentence_id','word_count','star_rating']].astype(int)
...
df_aws_sentences.to_sql('sa_review_sentences_test', engine,schema='Logi_Vc_Logs' ,index=False,if_exists='append')
df_word_freq.to_sql('sa_word_freqs_test',engine,schema='Logi_Vc_Logs',index=False,if_exists='append')
################
...
### the next step is to merge the tables 




import psycopg2
#connect to database (denodo)
try:
    nconn = psycopg2.connect("dbname='postgres' user='logi_mobile' host='aws13circleanalytics.clk8xdvewu4b.us-west-2.rds.amazonaws.com' password='logi_mobile' port='5432' \
    connect_timeout=0")
except:
...
def merge_tables_query(inityear,initmonth,initday,endyear,endmonth,endday):
    if(len(str(initday))==1):
        str_day="0"+str(initday)
    else:
        str_day=str(initday)
    ...
    if(len(str(endmonth))==1):
        ...
        estr_month=str(endmonth)
    quer="""insert into "Logi_Vc_Logs".amazon_review_sentiment_text
    select rs.*, word, translated_word, freq
    from "Logi_Vc_Logs".sa_word_freqs_test wf
    inner join "Logi_Vc_Logs".sa_review_sentences_test rs
    on rs.reviewsentence_wid = wf.reviewsentence_wid where rs.review_posted_date"""
    quer+=" > to_timestamp("+"'"+str_day+'-'+str_month+'-'+str(inityear)+' 00:00:01'+"'"
    quer+=", "
    quer+="\'"+"dd-mm-yyyy hh24:mi:ss"
    quer+="\'"
    quer+=")"
    #upper
    quer+=" and "
    quer+="rs.review_posted_date"
    quer+=" <= to_timestamp("+"'"+estr_day+'-'+estr_month+'-'+str(endyear)+' 00:00:00'+"'"
    quer+=", "
    quer+="\'"+"dd-mm-yyyy hh24:mi:ss"
    quer+="\'"
    quer+=")"
    quer+=" order by review_posted_date, reviewsentence_wid"
    return quer
    
    
merge_query=merge_tables_query(newstart.year,newstart.month,newstart.day,end_day.year,end_day.month,end_day.day)
ncur=nconn.cursor()
ncur.execute(merge_query)
nconn.commit()
nconn.close()

    
# insert into "Logi_Vc_Logs".amazon_review_sentiment
# select rs.*, word, translated_word, freq
# from "Logi_Vc_Logs".sa_word_freqs wf
# inner join "Logi_Vc_Logs".sa_review_sentences rs
# on rs.reviewsentence_wid = wf.reviewsentence_wid where rs.review_posted_date=t
# order by review_posted_date, reviewsentence_wid
