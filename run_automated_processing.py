...
myque=make_quer_date(mycolumns,newstart.year,newstart.month,newstart.day,2,newend.year,newend.month,newend.day)
cur.execute(myque)
...
def get_sentiment_en(sentence):
    ...
    sentiment_pol=TextBlob(sentence).sentiment.polarity
...
################################
# writing the results to the log table
newid=int(last_id)+1
newjobname='process_review'
newstatus='end'
newuser='ayaqubov'

newdata=(newid,newjobname,newstatus,newstart,newend,newuser)
...
try:
    cur.execute(insert_stmt, newdata)
